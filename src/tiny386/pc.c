...
static void pc_io_write(void *o, int addr, u8 val)
{
	...
	case 0x402:
...
#endif
		{
			...
			char ch = (char)val;
			if (ch == boot_msg[pc->debugcon_boot_match]) {
				pc->debugcon_boot_match++;
				if (boot_msg[pc->debugcon_boot_match] == '\0') {
					pc->boot_sector_seen = true;
					fprintf(stderr, "[splash] debugcon matched boot sector: %s\n",
						boot_msg);
				}
			} else {
				pc->debugcon_boot_match = ch == boot_msg[0] ? 1 : 0;
			}
		}
		return;
	case 0x92:
	   ...
	   u8 *fb, PCConfig *conf)
{
	...
	pc->boot_sector_seen = false;
	pc->debugcon_boot_match = 0;
